...
class Trainer:
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes, dataset, run_dir="./"):
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, device):
        logger.add(save_dir.replace("models", "logs") + "_{time}.log")
        ...
        num_examples = len(batch_gen.list_of_examples)
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                ...
                for p in predictions:
                    ce = self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))
                    sm = 0.15*torch.mean(torch.clamp(self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(p.detach()[:, :, :-1], dim=1)), min=0, max=16)*mask[:, :, 1:])
                    loss += ce + sm
                    ce_loss += ce
                    sm_loss += sm
            ...
            logger.info("[epoch %d]: lr = %f, epoch loss = %f (ce loss = %f, smoothing loss = %f),   acc = %f" % (epoch + 1, optimizer.param_groups[0]['lr'], 
                                                                                                                epoch_loss / num_examples,
                                                                                                                ce_epoch_loss / num_examples,
                                                                                                                sm_epoch_loss / num_examples,
                                                                                                                float(correct)/total))
            scheduler.step()

    def predict(self, checkpoint, results_dir, features_path, vid_list_file, actions_dict, device, sample_rate):
        self.model.eval()
        with torch.no_grad():
            self.model.to(device)
